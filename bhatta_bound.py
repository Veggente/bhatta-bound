...
def check_conjecture_higher_dim(  # pylint: disable=too-many-branches
    ...
    plt.savefig(f"conjecture-d{num_switch_vals}-n{num_vals}-nst{no_small_terms}.eps")
    return True
...
def new_roc_bound(
    rho_sq_list: list[float], fpr: list[float], prob_dist: Optional[list[float]] = None
) -> np.ndarray:
    """Calculates our new ROC upper bound.

    Args:
        rho_sq_list: Bhattacharyya coefficients squared.
        fpr: FPR in numerical estimation.
        prob_dist: Probability distribution of the BCs.  If None,
    ...
    for pi in np.linspace(0.01, 0.99, 99):  # pylint: disable=invalid-name
        ...
        if prob_dist is None:
            ...
            new_bound = prob_dist @ np.array(bound_per_bc)
...
def nonconcavity():
    ...
    g_2 = f_2
    f_func = 2 / 3 * f_1 + 1 / 3 * f_2
    g = 2 / 3 * g_1 + 1 / 3 * g_2  # pylint: disable=invalid-name
    lb_mixed = (
        1 / 2 * (1 - np.sqrt(1 - 4 * pi_list * (1 - pi_list) * bhatta_coeff(f_func, g)))
    )
    lb_component = (
        1
        / 2
